...
func generateKemonoConceptsPrompt(fieldID int) (messages ChatMessages, err error) {
	/*
		あなたには、かわいいマスコットやケモノやマモノたちだけが生息する世界観のゲームの、ゲーム内システムを担当してもらいます。

		森に住んでいるケモノのキャラクターの概念を考えて欲しいです。
		概念は、以下のような単語や短い文で表されます。属性のような概念です。以下は概念の例です。
		- かわいい
		- マスコット
		- 四足歩行
		- ポケモン
		...
		この概念は、キャラクターの画像の生成に使ったり、キャラクターの説明の生成に使うので、使いやすそうな単語や短い文を考えて欲しいです。概念を8個考えて、概念だけ出力してください。概念の説明は必要ありません。8個の中の1つは、属性にしてください。4つは、キャラクターの特徴を表すものにしてください。属性や特徴を考える際は、ケモノが住んでいる場所を考慮したうえで考えてください。例に挙げていない属性でも大丈夫です。ユニークな属性だとより面白いです。
	*/
	/*
		- 透明な羽
		- 輝く瞳
	...
	*/
	/*
		完璧です！また別のケモノキャラクターの情報を送るので、まったく同じようにして、今度は%sに住んでいるケモノのキャラクターの概念を考えて欲しいです。
		概念は、以下のような単語や短い文で表されます。属性のような概念です。以下は概念の例です。
		- かわいい
		- マスコット
		- 四足歩行
	...
	*/
	...
	err = messages.AddAssistantMessageContent("- 透明な羽\n- 輝く瞳\n- 音の魔術師\n- 草属性\n- ささやく声\n- 木の上で眠る\n- 遊び好き\n- 幻想的な尾")
	if err != nil {
		return
	}

	var promptTexts []string
	promptTexts = append(promptTexts, fmt.Sprintf("完璧です！また別のケモノキャラクターの情報を送るので、まったく同じようにして、今度は%sに住んでいるケモノのキャラクターの概念を考えて欲しいです。", domains.FieldIdToFieldType(fieldID).String()))
	promptTexts = append(promptTexts, "概念は、以下のような単語や短い文で表されます。属性のような概念です。以下は概念の例です。\n- かわいい\n- マスコット\n- 四足歩行\n- ポケモン\n- 色が鮮やか\n- 炎属性\n- 水属性\n- 草属性\n- 空属性\n- 風属性\n- オレンジ色の毛皮\n- スライム\n- 丸呑み\n- 触手を持つ\n- 毒をもつ\n- ふわもこ質感\n- パステルカラー\n- 果物を好む\n- 美しい\n- かっこいい\n- 木登りの達人\n\nこの概念は、キャラクターの画像の生成に使ったり、キャラクターの説明の生成に使うので、使いやすそうな単語や短い文を考えて欲しいです。概念を8個考えて、概念だけ出力してください。概念の説明は必要ありません。8個の中の1つは、属性にしてください。4つは、キャラクターの特徴を表すものにしてください。属性や特徴を考える際は、ケモノが住んでいる場所を考慮したうえで考えてください。例に挙げていない属性でも大丈夫です。ユニークな属性だとより面白いです。")
